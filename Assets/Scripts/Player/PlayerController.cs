...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField]
    [Tooltip("transform of camera following the player ")]
    ...
    // The current move direction, does not include magnitude
    private Vector2 p_Velocity;

    //In order to do anything, we can not be frozen
    private float p_FrozenTimer;

    //Default Color. Cached so we can switch between colors
    private Color p_DefaultColor;

    //Current amount of health player has
    ...
    private void Awake()
    {
        ...
        for (int i=0; i<m_Attacks.Length; i++)
        {
            ...
            {
                Debug.LogError(attack.AttackName + "has a wind up time larger than player frozen time");
            }
        }
    }
    ...
    private void Update()
    {
        ...
        // set how hard player is pressing the button
        ...
        if (forward > 0 && forward < threshold)
        {
            forward = 0;
        } else if (forward < 0 && forward > -threshold)
        {
            forward = 0;
        }
